...
| Session chat core (Scope B1) | `amux-server.py` | `chat_messages`, `chat_replies`, `_chat_extract_turns`, `_chat_populate_replies`, `_chat_build_thread`, `_chat_insert_owner`, `_chat_insert_system`, `_chat_delivery_status`, `_chat_reconcile_all`, `_chat_notify`, `_chat_replies_lock`, `_chat_limit_notified`, `AMUX-LOCAL:session-chat`, `/api/chat` | Sentinel-wrapped (11 balanced `AMUX-LOCAL:session-chat` block fences + inline landmarks; `grep -c AMUX-LOCAL:session-chat` = 26) | Schema tables in `_DB_SCHEMA` after `steering_history` (`-- AMUX-LOCAL:session-chat` fence); helper block after `get_db()` (`# AMUX-LOCAL:session-chat`); `POST/GET /api/chat` in `_route_inner` after the `/api/tunnel/stop` block; `_classify_request` chat entry before `# System`; monitor idle-hook `_chat_populate_replies(sname)` inside `_on_idle`; SSE `chat` emit after the alerts push + reconcile-on-connect before the `_sse_events` `while True`; usage-limit `_chat_insert_system` in `_rate_limit_auto_respond` after `_session_auto_actions.setdefault` + `_chat_limit_notified.discard` re-arms; dashboard `_chatOnSSE`/`_chatPoll` block + fallback wiring in `enablePollingFallback` | Keep `chat_messages` as an OWNER/SYSTEM-input-only log (no `delivered` column, no `role='session'` rows); keep `chat_replies` a rebuildable materialized index (rebuild via `DELETE FROM chat_replies`, NEVER `DROP TABLE` — C-crit-2); keep single-writer `_chat_populate_replies` under `_chat_replies_lock` inserting in ascending `turn_index` (C-new-1); keep the `isSidechain` subagent filter in the pure `_chat_extract_turns`; owner input goes through `POST /api/chat` → `_steer_enqueue` only, never `cmd_history`; `GET /api/chat` stays read-loose (peek parity); keep the `chat` SSE type in BOTH `_sse_events` and the dashboard polling fallback | **Y** — generic per-session chat over the existing steering/SSE surface; upstreamable |
