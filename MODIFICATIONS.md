...
each delta lands once the fork re-baselines onto Rust. Phase P3's gate requires **zero
in-file Rust deltas** — anything that can't clear config or sidecar placement stops for the
user's decision rather than getting grafted into `crates/` by default (see
[`.claude/rules/extend-via-sidecar.md`](.claude/rules/extend-via-sidecar.md)).
...
| `AMUX_COMMIT_STAMP` toggle | Code delta in `amux-server.py` | **Open — sidecar/wrapper if possible** | Not yet decided; flagged in the plan as a phase-P3 open item. |
| Yolo-guardrail | Code delta in `amux-server.py` | **Open — sidecar if possible** | Not yet decided; flagged in the plan as a phase-P3 open item. |
...
| Yolo-guardrail — never auto-approve permission-rule prompts | `amux-server.py` | `AMUX-LOCAL:yolo-guardrail`, `_yolo_is_guarded_prompt`, `_YOLO_GUARD_DESTRUCTIVE`, `_yolo_guard_logged`, `yolo_guardrail_skipped` | Sentinel-wrapped (`AMUX-LOCAL:yolo-guardrail`, two fenced blocks — the helper/matcher above `_yolo_auto_respond`, and the skip check inside its per-session loop; `grep -c AMUX-LOCAL:yolo-guardrail amux-server.py` = 4) | `_yolo_auto_respond` — the guard block sits immediately before the `for i, (pattern, response) in enumerate(_YOLO_PROMPTS)` send loop; if `_yolo_is_guarded_prompt(clean)` returns true it `continue`s (leaves the session `waiting`) instead of `send_text(name, response)`. Helper + `_YOLO_GUARD_DESTRUCTIVE` regex defined just after `_yolo_last_responded` | Keep the destructive-command stand-down: when the pane shows Claude Code's `Permission rule … requires confirmation` chrome (primary signal — an `ask` rule fired) OR a destructive command inside a `Do you want to proceed?` prompt (secondary belt: `sudo`/`rm -rf|-fr`/`git push -f|--force`/`dd if=`/`mkfs`/`git clean -fdx|-xfd`), do NOT auto-answer — let it stay `waiting` for a human (Phase B Telegram notify). Manually kept in sync with the `permissions.ask` globs in each `settings.json` (different process/file — cannot share a literal; cross-ref comment lives in the Python block). Graft around whatever upstream changes to the `_yolo_auto_respond` send site | Inert — upstreaming declined. Was: **Y** — candidate upstream PR: "yolo auto-responder skips a configurable destructive-command set" / never auto-approve explicit permission-rule confirmations |
