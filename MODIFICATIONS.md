...
| Session chat (Scope B1 core + B2 dashboard tab) | `amux-server.py`, `chat.js`, `chat.css` | `chat_messages`, `chat_replies`, `_chat_extract_turns`, `_chat_populate_replies`, `_chat_build_thread`, `_chat_insert_owner`, `_chat_insert_system`, `_chat_delivery_status`, `_chat_reconcile_all`, `_chat_notify`, `_chat_replies_lock`, `_chat_limit_notified`, `_chatOnSSE`, `_chatPoll`, `_chatTabOpen`, `_chatTabClose`, `peek-tab-chat`, `peek-chat-panel`, `AMUX-LOCAL:session-chat`, `/api/chat`, `/chat.js`, `/chat.css` | Sentinel-wrapped (balanced `AMUX-LOCAL:session-chat` block fences — Python `#`, JS `//`, SQL `--`, HTML `<!-- -->` — plus a few open-only inline markers matching B1 style; `grep -c AMUX-LOCAL:session-chat` = 37). **B2 feature code lives in referenced files `chat.js`/`chat.css` (upstream has neither → conflict-immune); the in-file B2 footprint is ~15 functional lines.** | **B1:** schema tables in `_DB_SCHEMA` after `steering_history` (`--` fence); helper block after `get_db()` (`#`); `POST/GET /api/chat` in `_route_inner` after `/api/tunnel/stop`; `_classify_request` chat entry before `# System`; monitor idle-hook `_chat_populate_replies(sname)` inside `_on_idle`; SSE `chat` emit after the alerts push + reconcile-on-connect before the `_sse_events` `while True`; usage-limit `_chat_insert_system` in `_rate_limit_auto_respond`; dashboard `_chatOnSSE`/`_chatPoll` block + fallback wiring in `enablePollingFallback`. **B2:** `<link href="/chat.css">` after the xterm CSS `<link>`; `<script src="/chat.js" defer>` after the fullcalendar `<script>`; `peek-tab-chat` button after `peek-tab-terminal` in `.peek-tabs`; `<div id="peek-chat-panel">` after `peek-messages-panel`; chat dispatch appended in `setPeekTab` after the notes-panel block; `_chatTabClose()` call in `closePeek` after `peekSession = null`; `/chat.js`+`/chat.css` static route after the `/sw.js` route; `/chat.js`+`/chat.css` added to `_PUBLIC_PATHS`. | **B1:** keep `chat_messages` an OWNER/SYSTEM-input-only log (no `delivered` column, no `role='session'` rows); keep `chat_replies` a rebuildable materialized index (rebuild via `DELETE FROM chat_replies`, NEVER `DROP TABLE` — C-crit-2); keep single-writer `_chat_populate_replies` under `_chat_replies_lock` inserting in ascending `turn_index` (C-new-1); keep the `isSidechain` subagent filter in the pure `_chat_extract_turns`; owner input goes through `POST /api/chat` → `_steer_enqueue` only, never `cmd_history`; `GET /api/chat` stays read-loose (peek parity); keep the `chat` SSE type in BOTH `_sse_events` and the dashboard polling fallback. **B2:** keep the UI in `chat.js`/`chat.css` (referenced files) with only the tiny sentinel footprint in-file; `chat.js` consumes the B1 `amux:chat-thread`/`amux:chat` CustomEvents + `window._chatActiveSession`/`_chatCursor`/`_chatPoll` (merge-by-id dedups SSE+poll overlap); composer writes via `apiCall` → `POST /api/chat` carrying `X-Amux-Write-Token` (never bypass Scope A); the chat tab is a new peek-overlay tab (Chat) beside Terminal/Messages — raw terminal stays in Terminal. | **Y** — generic per-session chat over the existing steering/SSE surface; upstreamable |
