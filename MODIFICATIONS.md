...
| `amux` CLI — remote-control/model defaults & yolo-by-default (Hunks 6–7) | `amux` | `AMUX_NO_REMOTE_CONTROL`, `AMUX_DEFAULT_MODEL`, `claude-opus-4-8[1m]`, `AMUX_NO_YOLO` | Not sentinel-wrapped (lazy retrofit) | `cmd_start` claude-branch, `else` of the `provider == "codex"` check (see Hunks 6–7 below) | Keep the MODIFICATIONS.md hunks (yolo default, default model, remote-control) AND upstream's new commands — both sides | N (personal defaults — Opus-1M, yolo, remote-control — not pursued upstream) |
...
| `AMUX_DEFAULT_MODEL` | `claude-opus-4-8[1m]` | Model passed to `claude` when the session didn't set its own `--model` |
...
```bash
    ...
    local default_model="${AMUX_DEFAULT_MODEL:-claude-opus-4-8[1m]}"
...
2. **Default model = `claude-opus-4-8[1m]`** (Opus 4.8 with 1M context window) instead of upstream's `sonnet`. Brackets are escape-safe via `printf '%q'`. Override per-session by passing `--model X` to `amux register`, or globally with `AMUX_DEFAULT_MODEL`.
...
cmd="claude"
[[ "${AMUX_NO_REMOTE_CONTROL:-}" == "1" ]] || [[ "$cmd" == *"--remote-control"* ]] || cmd="$cmd --remote-control"
default_model="${AMUX_DEFAULT_MODEL:-claude-opus-4-8[1m]}"
[[ "$cmd" == *"--model"* ]] || cmd="$cmd --model $(printf '%q' "$default_model")"
echo "$cmd"
T
